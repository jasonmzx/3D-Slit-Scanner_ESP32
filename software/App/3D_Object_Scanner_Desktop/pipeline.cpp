...
VerticeObject pipeline1(std::string dataset, int midpoint, int cutoff) {
    ...
    for (LazerSlice slice : preprocessed_dataset) {
        ...
        float debug_n = slice.angle / 360.0;

        for (glm::vec3 point : slice.list_3d_points) {

            xyz_slice.reserve(6);

            auto bottom_left_back_normal = glm::normalize(glm::vec3(-1, -1, -1));
            auto bottom_right_back_normal = glm::normalize(glm::vec3(1, -1, -1));
            auto top_right_back_normal = glm::normalize(glm::vec3(1, 1, -1));
            auto top_left_back_normal = glm::normalize(glm::vec3(-1, 1, -1));
            auto bottom_left_front_normal = glm::normalize(glm::vec3(-1, -1, 1));
            auto bottom_right_front_normal = glm::normalize(glm::vec3(1, -1, 1));
            auto top_right_front_normal = glm::normalize(glm::vec3(1, 1, 1));
            auto top_left_front_normal = glm::normalize(glm::vec3(-1, 1, 1));


            xyz_slice.insert(xyz_slice.end(), {
                // Position, Normal, Other Attributes
                point[0] - cubeSize, point[1] - cubeSize, point[2] - cubeSize, bottom_left_back_normal.x, bottom_left_back_normal.y, bottom_left_back_normal.z, debug_n, 1.0f, debug_n, // Vertex 1 (bottom-left-back)
                point[0] + cubeSize, point[1] - cubeSize, point[2] - cubeSize, bottom_right_back_normal.x, bottom_right_back_normal.y, bottom_right_back_normal.z, debug_n, 1.0f, debug_n, // Vertex 2 (bottom-right-back)
                point[0] + cubeSize, point[1] + cubeSize, point[2] - cubeSize, top_right_back_normal.x, top_right_back_normal.y, top_right_back_normal.z, debug_n, 1.0f, debug_n, // Vertex 3 (top-right-back)
                point[0] - cubeSize, point[1] + cubeSize, point[2] - cubeSize, top_left_back_normal.x, top_left_back_normal.y, top_left_back_normal.z, debug_n, 1.0f, debug_n, // Vertex 4 (top-left-back)
                point[0] - cubeSize, point[1] - cubeSize, point[2] + cubeSize, bottom_left_front_normal.x, bottom_left_front_normal.y, bottom_left_front_normal.z, debug_n, 1.0f, debug_n, // Vertex 5 (bottom-left-front)
                point[0] + cubeSize, point[1] - cubeSize, point[2] + cubeSize, bottom_right_front_normal.x, bottom_right_front_normal.y, bottom_right_front_normal.z, debug_n, 1.0f, debug_n, // Vertex 6 (bottom-right-front)
                point[0] + cubeSize, point[1] + cubeSize, point[2] + cubeSize, top_right_front_normal.x, top_right_front_normal.y, top_right_front_normal.z, debug_n, 1.0f, debug_n, // Vertex 7 (top-right-front)
                point[0] - cubeSize, point[1] + cubeSize, point[2] + cubeSize, top_left_front_normal.x, top_left_front_normal.y, top_left_front_normal.z, debug_n, 1.0f, debug_n  // Vertex 8 (top-left-front)
             });
...
VerticeObject pipeline1_cutoff(std::string dataset, int midpoint, int cutoff) {
    std::cout << "\n Running pipeline 1...\n";
...
VerticeObject pipelineCombo(std::string set_of_datasets_path, int midpoint, int cutoff) {
    ...
    for (std::vector<LazerSlice> preprocessed_dataset : set_of_preprocessed_datasets) {
        ...
        for (LazerSlice slice : preprocessed_dataset) {
            extract_cylindrical_pts__rot_mat(slice, cameraMatrix, distCoeffs, newCameraMatrix, (i*120.0f));
            std::cout << "Sl3D: " << slice.list_3d_points.size() << std::endl;
